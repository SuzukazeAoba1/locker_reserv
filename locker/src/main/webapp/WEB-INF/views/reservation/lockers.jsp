...
<c:url var="lockerDetailBase" value="/reservation/lockers"/>

<!-- 위치 선택 링크 -->
<nav class="location-nav">

<c:choose>
<c:when test = "${param.location =='西改札口券売機前'}">
<c:url var="url1" value="/reservation/lockers"><c:param name="location" value="西改札口券売機前"/></c:url>
</c:when>

<c:when test = "${param.location =='西改札口横'}">
<c:url var="url2" value="/reservation/lockers"><c:param name="location" value="西改札口横"/></c:url>
</c:when>

<c:when test = "${param.location =='中央改札口横'}">
<c:url var="url3" value="/reservation/lockers"><c:param name="location" value="中央改札口横"/></c:url>
</c:when>

<c:when test = "${param.location =='東口改札口・5番出入口前'}">
<c:url var="url4" value="/reservation/lockers"><c:param name="location" value="東口改札口・5番出入口前"/></c:url>
</c:when>

<c:when test = "${param.location =='東口改札口出口付近'}">
<c:url var="url5" value="/reservation/lockers"><c:param name="location" value="東口改札口出口付近"/></c:url>
</c:when>

<c:when test = "${param.location =='12番出入口前'}">
<c:url var="url6" value="/reservation/lockers"><c:param name="location" value="12番出入口前"/></c:url>
</c:when>

<c:when test = "${param.location =='お客様サービスセンター横'}">
<c:url var="url7" value="/reservation/lockers"><c:param name="location" value="お客様サービスセンター横"/></c:url>
</c:when>
